...
// Type: EMx.UI.Dialogs.WaitingRunnableDialogNew
...
{
  public partial class WaitingRunnableDialogNew : Window, IComponentConnector
  {
    ...
    public WaitingRunnableDialogNew()
    {
    ...
    {
      this.ctrlProgress.Value = this.RunnableObject.Progress.Value;
      if (!(this.stackedMsg != ""))
        return;
      lock (this.stackedMsg)
      {
        this.txtLog.AppendText(this.stackedMsg);
    ...
    {
      if (this.RunnableObject == null || this.RunnableObject.IsRunning)
        return;
    ...
    private void btnStop_Click(object sender, RoutedEventArgs e)
    {
      ...
      else if (this.RunnableObject.IsRun)
      {
        ...
        else
        {
          if (this.PushStopButtonCount <= this.TryCountToStop)
            return;
          WaitingRunnableDialogNew.log.Warn("Abort Runnable[{0}] by user.", (object) this.RunnableObject.Name);
    ...
    private void AppendText(string fmt, params object[] args)
    {
      ...
      WaitingRunnableDialogNew.log.Debug((Action<LoggingToken>) (x =>
      {
        x.Message = msg;
        x.StackHops = 1;
      }));
      lock (this.stackedMsg)
        this.stackedMsg = this.stackedMsg + msg + Environment.NewLine;
    }
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent((object) this, new Uri("/EMx.UI;component/dialogs/waitingrunnabledialognew.xaml", UriKind.Relative));
    }
